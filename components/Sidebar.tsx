    ...
    <>
      <header className="lg:hidden fixed top-0 left-0 right-0 z-30 h-14 px-4 flex items-center justify-between bg-white border-b border-ink-100">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-brand-500 flex items-center justify-center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M9 12 Q9 7 12 7 Q15 7 16 9 M9 12 Q9 17 12 17 Q15 17 16 15" stroke="white" strokeWidth="2.5" strokeLinecap="round"/></svg></div>
      ...
      <aside className={`fixed top-0 left-0 z-20 h-screen w-64 bg-white border-r border-ink-100 transition-transform ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="hidden lg:flex items-center gap-3 h-16 px-6 border-b border-ink-100">
          <div className="w-9 h-9 rounded-xl bg-brand-500 flex items-center justify-center">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M9 12 Q9 7 12 7 Q15 7 16 9 M9 12 Q9 17 12 17 Q15 17 16 15" stroke="white" strokeWidth="2.5" strokeLinecap="round"/></svg>
          </div>
          <div>
          ...
          {NAV.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition ${active ? 'bg-brand-50 text-brand-700' : 'text-ink-700 hover:bg-ink-50 hover:text-ink-900'}`}>
                <Icon name={item.icon} />
                {item.label}
              </Link>
            );
