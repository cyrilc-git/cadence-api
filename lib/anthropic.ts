...
Police system-ui sans-serif. Coins arrondis 16px sur cartes, 10px sur boutons. Style épuré, espace blanc, hiérarchie typo claire. Pas de dégradés tape-à-l'œil. Pas d'emojis sur les visuels produit.
...
export async function generateClaudeDesignSvg(prompt: string): Promise<{ svg: string; model: string }> {
  const c = await client();
  const MODEL = 'claude-sonnet-4-6';
  const msg = await c.messages.create({ model: MODEL, max_tokens: 4000, system: VISUAL_SYSTEM_PROMPT, messages: [{ role: 'user', content: prompt }] });
  const raw = msg.content.filter((x: any) => x.type === 'text').map((x: any) => x.text).join('\n');
