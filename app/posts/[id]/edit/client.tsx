...
const QUICK_ACTIONS = [
  ...
  { label: 'Améliorer la mise en forme',  prompt: 'Améliore la mise en forme LinkedIn : paragraphes courts, espaces, hook fort, mots-clés en gras Unicode si pertinent (𝗯𝗼𝗹𝗱), pas de tiret long, pas d\'em dash, hashtags ciblés à la fin.' },
  { label: 'Nettoyer style IA',           prompt: 'Retire tout style IA reconnaissable : phrases creuses, formules signature, mots interdits (seamless, robust, game changer, révolutionner, booster, libérer le potentiel, dans un monde où, la vérité c\'est que, spoiler, Et vous ?).' },
  { label: 'Proposer une illustration',   prompt: 'Propose un brief d\'illustration pour ce post : style design system Heelio (couleurs #6366F1, #4F46E5, fond #F8FAFC), format 1200x630. Décris ce qu\'on devrait voir.' }
];
      ...
      <header className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-ink-500">
            ...
            {validated ? <span className="ml-2 text-success-700">● validé</span> : <span className="ml-2 text-warn-700">⚠ non validé</span>}
          </p>
        </div>
        ...
        <section className="space-y-4">
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <button onClick={() => wrap('','', (s) => toBold(s))}   className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50" title="Gras Unicode">𝗕</button>
              <button onClick={() => wrap('','', (s) => toItalic(s))} className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50" title="Italique Unicode">𝘐</button>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            ...
            {versions.length > 1 && (
              <button onClick={revert} className="mt-3 text-xs px-3 py-1.5 rounded-lg ring-1 ring-ink-300 hover:bg-ink-50">↶ Revenir à la version précédente ({versions.length - 1})</button>
            )}
          </div>
        </section>
