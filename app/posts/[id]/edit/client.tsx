...
const QUICK_ACTIONS = [
  { label: 'AmÃ©liorer le hook',          prompt: 'AmÃ©liore le hook : rends-le plus accrocheur, < 80 caractÃ¨res, factuel, sans clickbait.' },
  { label: 'Raccourcir',                 prompt: "Raccourcis ce post Ã  600-700 caractÃ¨res en prÃ©servant l'essentiel et un exemple chiffrÃ©." },
  { label: 'Rendre plus concret',        prompt: 'Rends ce post plus concret : ajoute un exemple chiffrÃ©, un cas anonymisÃ©, des paragraphes plus courts.' },
  { label: 'Adapter au vouvoiement',     prompt: 'VÃ©rifie le vouvoiement strict. Corrige tout tutoiement.' },
  { label: 'AmÃ©liorer la mise en forme', prompt: 'AmÃ©liore la mise en forme LinkedIn : paragraphes courts, hook fort, mots-clÃ©s en gras Unicode si pertinent (ð¯ð¼ð¹ð±), pas de tiret long, hashtags ciblÃ©s Ã  la fin.' },
...
export default function EditClient({ initial, validated: initialValidated }: { initial: { summary: any; content: string }; validated: boolean }) {
  ...
  async function save() {
    ...
    try {
      ...
      setSaveMsg(`SauvegardÃ© ${validated ? 'â (validÃ© pour cron)' : '(non validÃ©)'}`);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-ink-500">
            ...
            {validated ? <span className="ml-2 text-success-700">â validÃ©</span> : <span className="ml-2 text-warn-700">â  non validÃ©</span>}
          </p>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <section className="space-y-4">
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <button onClick={() => applyTransform(toBold)}   className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50" title="SÃ©lection en gras Unicode">ð</button>
              <button onClick={() => applyTransform(toItalic)} className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50" title="SÃ©lection en italique Unicode">ð</button>
              <button onClick={() => setText(text.replace(/[ââ]/g, ','))} className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50">Retirer tirets longs</button>
              <button onClick={() => setText(text.replace(/\n+/g, '\n\n').trim())} className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50">AÃ©rer paragraphes</button>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            <h3 className="font-semibold text-ink-900 text-sm">AmÃ©liorer avec l'IA</h3>
            ...
            <div className="mt-3 flex gap-2">
              <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && chatInput && runChat(chatInput)} placeholder="Ou tapez une instruction libreâ¦" className="flex-1 px-3 py-2 rounded-lg ring-1 ring-ink-300 text-sm" />
              <button onClick={() => chatInput && runChat(chatInput)} disabled={chatLoading || !chatInput} className="px-3 py-2 rounded-lg bg-brand-500 text-white text-sm font-medium hover:bg-brand-600 disabled:opacity-50">{chatLoading ? 'â¦' : 'Envoyer'}</button>
            </div>
            <div className="mt-4 max-h-96 overflow-y-auto space-y-3">
              {chat.length === 0 && <p className="text-xs text-ink-500 italic">Aucun message. Cliquez sur une suggestion ci-dessus pour dÃ©marrer.</p>}
              {chat.map((m, i) => (
            ...
            {versions.length > 1 && (
              <button onClick={revert} className="mt-3 text-xs px-3 py-1.5 rounded-lg ring-1 ring-ink-300 hover:bg-ink-50">â¶ Revenir Ã  la version prÃ©cÃ©dente ({versions.length - 1})</button>
            )}
          </div>
        ...
        <section className="space-y-4">
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            <h3 className="font-semibold text-ink-900 text-sm mb-3">AperÃ§u LinkedIn</h3>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20 space-y-2">
            <label className="flex items-start gap-3 cursor-pointer select-none p-2 rounded-lg ring-1 ring-ink-100 bg-warn-50/30">
              ...
              <span className="text-sm text-ink-700">
                <strong className="block text-ink-900">ValidÃ© pour publication automatique</strong>
                <span className="text-xs text-ink-500">Si cochÃ©, le cron quotidien publiera ce post Ã  l'heure programmÃ©e. Sinon il reste en draft.</span>
              </span>
            </label>
            <button onClick={save} disabled={saving} className="w-full px-4 py-2.5 rounded-lg bg-brand-500 text-white text-sm font-medium hover:bg-brand-600 disabled:opacity-50">{saving ? 'Sauvegardeâ¦' : 'Sauvegarder'}</button>
            <button onClick={() => setPublishOpen(true)} disabled={!text.trim()} className="w-full px-4 py-2.5 rounded-lg ring-1 ring-brand-500 text-brand-700 text-sm font-medium hover:bg-brand-50 disabled:opacity-50">Publier maintenantâ¦</button>
