...
const QUICK_ACTIONS = [
  { label: 'Am�liorer le hook',           prompt: 'Am�liore le hook : rends-le plus accrocheur, < 80 caract�res, factuel, sans clickbait.' },
  { label: 'Raccourcir',                  prompt: 'Raccourcis ce post � 600-700 caract�res en pr�servant l\'essentiel et un exemple chiffr�.' },
  { label: 'Rendre plus concret',         prompt: 'Rends ce post plus concret : ajoute un exemple chiffr�, un cas anonymis�, des paragraphes plus courts.' },
  { label: 'Adapter au vouvoiement',      prompt: 'V�rifie le vouvoiement strict. Corrige tout tutoiement.' },
  { label: 'Am�liorer la mise en forme',  prompt: 'Am�liore la mise en forme LinkedIn : paragraphes courts, espaces, hook fort, mots-cl�s en gras Unicode si pertinent (5�5�5�5�), pas de tiret long, pas d\'em dash, hashtags cibl�s � la fin.' },
  { label: 'Nettoyer style IA',           prompt: 'Retire tout style IA reconnaissable : phrases creuses, formules signature, mots interdits (seamless, robust, game changer, r�volutionner, booster, lib�rer le potentiel, dans un monde o�, la v�rit� c\'est que, spoiler, Et vous ?).' },
  { label: 'Proposer une illustration',   prompt: 'Propose un brief d\'illustration pour ce post : style design system Heelio (couleurs #6366F1, #4F46E5, fond #F8FAFC), format 1200x630. D�cris ce qu\'on devrait voir.' }
];
...
export default function EditClient({ initial, validated: initialValidated }: { initial: { summary: any; content: string }; validated: boolean }) {
  ...
  function applyRewrite(rewrite: string) {
    setVersions(v => [...v, rewrite]);
    setText(rewrite);
  }
  function revert() {
    ...
    v.pop();
    setVersions(v);
    setText(v[v.length - 1]);
  }

  function wrap(prefix: string, suffix: string, transform: (s: string) => string) {
    ...
    const replaced = transform ? transform(sel) : sel;
  ...
  async function save() {
    ...
    try {
      ...
      setSaveMsg(`Sauvegard� ${validated ? ' (valid� pour cron)' : '(non valid�)'}`);
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-ink-500">
            ...
            {validated ? <span className="ml-2 text-success-700">� valid�</span> : <span className="ml-2 text-warn-700">� non valid�</span>}
          </p>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <section className="space-y-4">
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <button onClick={() => wrap('','', (s) => toBold(s))}   className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50" title="Gras Unicode">5�</button>
              <button onClick={() => wrap('','', (s) => toItalic(s))} className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50" title="Italique Unicode">5</button>
              ...
              <button onClick={() => setText(text.replace(/\n+/g, '\n\n').trim())} className="text-xs px-2 py-1 rounded ring-1 ring-ink-300 hover:bg-ink-50">A�rer paragraphes</button>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            <h3 className="font-semibold text-ink-900 text-sm">Am�liorer avec l'IA</h3>
            ...
            <div className="mt-4 max-h-96 overflow-y-auto space-y-3">
              {chat.length === 0 && <p className="text-xs text-ink-500 italic">Aucun message. Cliquez sur une suggestion ci-dessus pour d�marrer.</p>}
              {chat.map((m, i) => (
            ...
            {versions.length > 1 && (
              <button onClick={revert} className="mt-3 text-xs px-3 py-1.5 rounded-lg ring-1 ring-ink-300 hover:bg-ink-50">� Revenir � la version pr�c�dente ({versions.length - 1})</button>
            )}
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20">
            <h3 className="font-semibold text-ink-900 text-sm mb-3">Aper�u LinkedIn</h3>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20 space-y-2">
            <label className="flex items-start gap-3 cursor-pointer select-none p-2 rounded-lg ring-1 ring-ink-100 bg-warn-50/30">
              ...
              <span className="text-sm text-ink-700">
                <strong className="block text-ink-900">Valid� pour publication automatique</strong>
                <span className="text-xs text-ink-500">Si coch�, le cron quotidien publiera ce post � l'heure programm�e. Sinon il reste en draft.</span>
              </span>
            </label>
