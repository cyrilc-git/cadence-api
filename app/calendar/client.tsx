...
const PILIER_COLORS: Record<string, string> = {
  ...
  'Lundi � Cas dirigeant anonymis�':                          'bg-blue-50 text-blue-700 ring-blue-500/20',
  'Mardi � P�dagogie':                                        'bg-green-50 text-green-700 ring-green-500/20',
  'Mardi � P�dagogie sans jargon':                            'bg-green-50 text-green-700 ring-green-500/20',
  'Mercredi � Produit':                                       'bg-purple-50 text-purple-700 ring-purple-500/20',
  'Mercredi � Produit / d�mo / nouveaut� / release note':     'bg-purple-50 text-purple-700 ring-purple-500/20',
  'Jeudi � Opinion':                                          'bg-orange-50 text-orange-700 ring-orange-500/20',
  'Jeudi � Opinion / hot take mesur�':                        'bg-orange-50 text-orange-700 ring-orange-500/20',
  'Vendredi � Build in public':                               'bg-pink-50 text-pink-700 ring-pink-500/20'
};

const MONTH_FR = ['Janvier','F�vrier','Mars','Avril','Mai','Juin','Juillet','Ao�t','Septembre','Octobre','Novembre','D�cembre'];
...
export default function CalendarClient({ initialPosts }: { initialPosts: any[] }) {
  ...
  async function generateWeek() {
    if (!confirm('G�n�rer 5 brouillons (lundi � vendredi) pour la semaine prochaine ? Aucune publication, juste des drafts NON valid�s.')) return;
    ...
    try {
      ...
      setMsg(`${created} brouillon(s) cr��(s) en NON valid�. Rechargez pour voir.`);
    } catch (e: any) {
    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => { const d = new Date(cursor); d.setMonth(d.getMonth() + 1); setCursor(d); }} className="px-3 py-1.5 rounded-lg ring-1 ring-ink-300 text-sm hover:bg-ink-50">�</button>
          <button onClick={generateWeek} disabled={generating} className="px-3 py-1.5 rounded-lg bg-brand-500 text-white text-sm font-medium hover:bg-brand-600 disabled:opacity-50">{generating ? 'G�n�ration&' : '( G�n�rer la semaine'}</button>
        </div>
      </header>
              ...
              return (
                <div key={k} className={`rounded-xl p-2 min-h-[120px] ring-1 ring-inset ${isToday ? 'ring-brand-500 bg-brand-50/40' : 'ring-ink-300/30 bg-white'} ${isOtherMonth ? 'opacity-40' : ''} ${isPast && !isToday ? 'opacity-70' : ''}`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {!isPast && <Link href={`/posts/new?date=${k}`} className="text-ink-400 hover:text-brand-500 text-xs leading-none" title="Cr�er un post">+</Link>}
                  </div>
                    ...
                    {items.map((p: any) => (
                      <Link key={p.id} href={`/posts/${p.id}/edit`}
                        ...
                        <span className="flex items-center gap-1">
                          {p.status === 'published' && <span title="Publi�"></span>}
                          {p.status !== 'published' && p.validated && <span title="Valid� pour cron" className="text-success-700">�</span>}
                          {p.status !== 'published' && !p.validated && <span title="� valider" className="text-warn-700">�</span>}
      ...
      <div className="bg-white rounded-2xl p-5 shadow-card ring-1 ring-inset ring-ink-300/20 text-sm text-ink-700">
        <div className="flex items-center gap-4 flex-wrap">
          <span className="flex items-center gap-2"><span className="text-warn-700">�</span> � valider (cron ne publie PAS)</span>
          <span className="flex items-center gap-2"><span className="text-success-700">�</span> Valid� (cron publiera � 7h30)</span>
          <span className="flex items-center gap-2"><span></span> Publi�</span>
        </div>
        <p className="mt-3 text-xs text-ink-500">Le cron Vercel ne publie QUE les drafts marqu�s comme valid�s via la page d'�dition. Aucune publication automatique d'un draft non valid�.</p>
      </div>
    </div>
  );
}
