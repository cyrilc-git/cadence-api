...
const KIND_LABEL: Record<string, { label: string; hint: string }> = {
  pilier:       { label: 'Piliers �ditoriaux', hint: 'Un pilier par jour de la semaine, ou par th�me.' },
  rule:         { label: 'R�gles de voix',     hint: 'Vouvoiement, founder voice, tonalit�&' },
  anti_pattern: { label: 'Interdictions',      hint: 'Tiret long, mots creux, formules � bannir.' },
  hashtag:      { label: 'Hashtags favoris',   hint: 'Hashtags cibl�s � privil�gier.' },
  cta:          { label: 'CTAs favoris',       hint: 'Phrases d\'appel � action r�currentes.' },
  hook:         { label: 'Hooks favoris',      hint: 'Premi�res phrases qui marchent.' },
  audience:     { label: 'Audiences',          hint: 'DAF, fondateurs PME, etc.' },
  format:       { label: 'Formats',            hint: 'Cas client, p�dagogie, opinion, build in public.' }
};
...
export default function BrandDnaClient({ initial, initialPlan = [] }: { initial: any[]; initialPlan?: any[] }) {
  ...
  async function restoreDefaults() {
    if (!confirm('Restaurer les valeurs par d�faut Cadence ? Cela ajoute uniquement les �l�ments manquants.')) return;
    setRestoring(true);
    try {
  ...
  async function generateWeek() {
    if (!confirm('G�n�rer 5 brouillons (lundi � vendredi) pour la semaine prochaine ? Aucune publication. Tous en NON valid�.')) return;
    ...
    try {
      ...
      setGenMsg(`${created} brouillon(s) cr��(s) en NON valid�.`);
    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-ink-500">Votre voix, vos r�gles, vos interdits. Modifiable, persistant.</p>
        </div>
        <button onClick={restoreDefaults} disabled={restoring} className="px-4 py-2 rounded-lg ring-1 ring-ink-300 text-sm font-medium hover:bg-ink-50 disabled:opacity-50">
          {restoring ? 'Restauration&' : '� Restaurer les valeurs par d�faut Cadence'}
        </button>
      </header>
      ...
      <section className="bg-white rounded-2xl p-6 shadow-card ring-1 ring-inset ring-ink-300/20">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <h2 className="font-semibold text-ink-900">Plan �ditorial de la semaine</h2>
            <p className="text-xs text-ink-500 mt-0.5">Chaque jour correspond � un pilier. Modifiable dans la section Piliers ci-dessous.</p>
          </div>
          ...
          {weekdayItems.map((slot: any) => (
            <div key={slot.weekday} className={`rounded-xl p-4 ring-1 ring-inset ${slot.pilier ? 'ring-brand-500/30 bg-brand-50/30' : 'ring-ink-300/30 bg-ink-50'}`}>
              ...
              <div className="mt-1 text-sm font-medium text-ink-900 leading-tight">
                {slot.pilier ? (slot.pilier.split('� ')[1] || slot.pilier) : <span className="text-ink-400 italic">Non d�fini</span>}
              </div>
            </div>
          ))}
        </div>
        {genMsg && <p className="mt-3 text-sm text-ink-700">{genMsg}</p>}
        <p className="mt-3 text-xs text-ink-500">Cliquez � G�n�rer ma semaine � pour cr�er 5 brouillons (lundi � vendredi) dans Notion, programm�s � 07:30. Chaque brouillon reste <strong>non valid�</strong> jusqu'� votre validation explicite dans la page d'�dition.</p>
      </section>
        ...
        return (
          <section key={kind} className="bg-white rounded-2xl p-6 shadow-card ring-1 ring-inset ring-ink-300/20">
            ...
            <ul className="mt-4 space-y-2">
              {list.length === 0 && <li className="text-sm text-ink-500 italic">Aucun �l�ment.</li>}
              {list.map(i => (
